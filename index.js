...
const CONTROL_EXPLAINERS = {
  ...
  measure: {
    title: 'Measure',
    body: `
      <h3>Why DL Points exist</h3>
      <p>A rank is a position, and positions don&rsquo;t add up. Being 18th in one
      ranking and 5th in another tells you where a school stands, but not how
      much it weighs, and it says nothing about a country at all. So we turned
      every position into points: 150 for 1st place, 149 for 2nd, down to 1 for
      150th. Positions become something you can add, share and compare.</p>
      ...
      <h3>What that unlocks</h3>
      <p>Add up a school&rsquo;s points across all five rankings and you get its
      <strong>total DL Points</strong>: one number that sets any university or
      school directly against any other, however differently they place from
      one ranking to the next. Open a school&rsquo;s card in the Universities &amp;
      Schools view to see it.</p>
      <p>Add up a country&rsquo;s schools and you get its <strong>Overall
      ...
      <p>Compare the two editions and you get <strong>Evolution
      2025-2026</strong>: who gained ground, and who lost it.</p>

      <h3>One rule throughout</h3>
      <p>Only the Top 150 of each ranking count. The Next 50 are there to
      explore, and never enter any score.</p>
...
/** DL Points as the card shows them: a median can land on a half point. */
const formatPoints = (value) =>
  Number.isInteger(value) ? value.toLocaleString() : value.toFixed(1);

/**
 * The one sentence under the median: above, below, level, or alone. A country
 * with a single ranked school has a median that is that school, and saying
 * "level with the median" there would be true and useless.
 */
function medianSentence(name, points, median, count, country) {
  const who = escapeHtml(name);
  const where = escapeHtml(country);
  if (count <= 1) {
    return `${who} is the only university or school from ${where} in the Top 150.`;
  }
  const standing = points > median ? '<strong>above</strong>'
    : points < median ? '<strong>below</strong>' : '<strong>level with</strong>';
  return `${who} sits ${standing} the median of the ${count} universities &amp;
    schools from ${where} in the Top 150.`;
}

/**
 * An institution's name without its bracketed parts, for use in a sentence:
 * "IE University represents...", not "IE University (incl. IE Business
 * School) represents...".
 */
...
function institutionSection(institutions, state) {
  ...
  return `
    ...
    <ul class="institution-list">
      ${scored.map(row).join('')}
      ${unranked.length ? `<li class="institution-divider">${
        state.selectedModule === 'global'
          ? 'Beyond the Global TOP 150'
          : `Beyond ${escapeHtml(MODULE_LABELS[state.selectedModule])} TOP 150`}</li>` : ''}
...
function renderInstitutionPanel(panel, context, state, data, institution, agg) {
  ...
  // nothing here either.
  // An institution's DL Points across every ranking, on the same basis as the
  // total shown in its card: Top 150 places only.
  const institutionPoints = (subject) => data.modules.reduce((sum, module) => {
    const rank = subject.ranks?.[edition]?.[module];
    const tier = subject.tier?.[edition]?.[module];
    return sum + ((rank && tier !== 'next50') ? (151 - rank) : 0);
  }, 0);

  // Where the school sits among the universities & schools of its own
  // country, as the median of their DL Points.
  //
  // This replaced a "share of the country's DL Points". A share works for a
  // country with a handful of schools (IE carries a third of Spain) and
  // collapses for one with many: every American school came out under 2%,
  // which says more about the size of the United States than about the
  // school. A median is read the same way everywhere: above it or below it.
  // It is the median rather than the mean because a few very high scores
  // pull a mean up and would put most schools "below average".
  //
  // Computed on the whole dataset, not on what the filters leave, so a Type
  // filter cannot move the reference point.
  const nationalScores = data.institutions
    .filter(other => other.country === institution.country && !other.next50Only)
    .map(institutionPoints)
    .filter(points => points > 0)
    .sort((a, b) => a - b);

  const nationalMedian = medianOf(nationalScores);
  ...
  panel.innerHTML = `
    ...
    ${ranked.length ? `
      ...
      <div class="total-points">
        ...
        <p class="total-points-raw"><strong>${totalPoints.toLocaleString()} DL Points</strong></p>

        ${nationalMedian !== null ? `
        <!-- 2. Where that total sits in its own country. -->
        <div class="total-points-share">
          <span class="total-points-label">${nationalScores.length > 1
            ? 'National median' : 'In'} · ${escapeHtml(institution.country)}</span>
          ${nationalScores.length > 1 ? `
          <p class="total-points-raw"><strong>${formatPoints(nationalMedian)} DL Points</strong></p>` : ''}
          <p class="total-points-sentence">${medianSentence(
            shortName(institution.name), totalPoints, nationalMedian,
            nationalScores.length, institution.country)}</p>
        </div>` : ''}

        <div class="score-explainer" hidden>
          <p>
            Every ranking places 150 universities &amp; schools, and each place is
            worth DL Points: 150 for 1st, down to 1 for 150th. Adding a school&rsquo;s
            points across all five rankings gives its total.
          </p>
          <p>
            The national median is the middle score among the universities &amp;
            schools ranked in the same country: half score more, half score less.
            It shows at a glance whether a school sits above or below what is usual
            at home. We use the median rather than the average because a few very
            high scores pull an average up.
          </p>
        </div>
    ...
    ${(competitorRows || globalRows) ? `
      ...
      <details class="method">
        ...
        <p class="method-formula">
          ...
          distance closeness = 1 ÷ (1 + km apart ÷ ${DISTANCE_SCALE})<br>
          <strong>score = ${Math.round(COMPETITOR_RANK_WEIGHT * 100)}% rank +
