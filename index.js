...
/** A share as the card shows it: whole percent, one decimal under 10%. */
const formatShare = (percent) =>
...
function renderInstitutionPanel(panel, context, state, data, institution, agg) {
  ...
  const countryPoints = data.institutions
    .filter(other => other.country === institution.country && !other.next50Only)
    .reduce((sum, other) => sum + institutionPoints(other), 0);

  const countryShare = countryPoints > 0 && institutionPoints(institution) > 0
    ? (institutionPoints(institution) / countryPoints) * 100 : null;
    ...
    ${ranked.length ? `
      ...
      <div class="total-points">
        ...
        <div class="score-explainer" hidden>
          ...
          <p>
            On its own that total says little. Set against everything its country
            earns, it says how much of that country&rsquo;s standing rests on this one
            school: a high share means the country depends on it, a low one means
            it is one strong school among several.
          </p>
        </div>
