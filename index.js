...
/**
 * The one sentence under the median. It names the pool the median is taken
 * from and passes no verdict: the two figures above it already show where the
 * school sits, and "below the median" would read as a judgement. A country
 * with a single ranked school gets its own sentence, since its "median" is
 * just that school.
 */
function medianSentence(name, points, median, count, country) {
  ...
  return `Median of the <strong>${count}</strong> universities &amp; schools from
    ${where} in the Top 150.`;
}
